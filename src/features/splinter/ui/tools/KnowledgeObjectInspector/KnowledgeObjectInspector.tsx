...
export const KnowledgeObjectInspector = ({ selectedText }: KnowledgeKnowledgeObjectInspectorProps): React.JSX.Element | null => {
  ...
  return (
    <div className="p-3 bg-muted/90 border border-border rounded-lg shadow-lg backdrop-blur-sm max-w-sm">
      <div className="flex items-center gap-2 mb-2 pb-2 border-b border-border/50">
        {getIcon()}
        <span className="font-mono text-sm font-semibold text-foreground">
          {objectDetails.name}
        </span>
        <span className="ml-auto text-2xs uppercase tracking-wider text-muted-foreground bg-card/50 px-1.5 py-0.5 rounded">
          {objectDetails.type}
        </span>
      </div>
      ...
      <div className="space-y-3">
        {objectDetails.definition && (
          <div>
            <div className="text-2xs uppercase text-muted-foreground mb-1">Definition</div>
            <div className="font-mono text-xs text-foreground bg-card/50 p-2 rounded break-all">
              {objectDetails.definition}
            </div>
          </div>
        )}
        ...
        {objectDetails.args && objectDetails.args.length > 0 && (
          <div>
            <div className="text-2xs uppercase text-muted-foreground mb-1">Arguments</div>
        ...
        {objectDetails.fields && objectDetails.fields.length > 0 && (
          <div>
            <div className="text-2xs uppercase text-muted-foreground mb-1">Fields</div>
            <div className="flex flex-wrap gap-1">
